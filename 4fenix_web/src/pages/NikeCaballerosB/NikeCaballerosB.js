...
function NikeCaballerosB() {
    ...
    const handleSubmit = async (event) => {
        ...
        if (validateForm()) {
            try {
                let url = 'http://localhost:3030/producto';
                let method = 'POST';

                if (editandoProducto) {
                    url += `/${editandoProducto.Id_producto}`;
                    method = 'PUT';
                }
    ...
    const handleEditarProducto = (productoId) => {
        const productoAEditar = productos.find(producto => producto.Id_producto === productoId);
        setNuevoProducto({
            ...productoAEditar
        });
            ...
            <div className="contenedor">
                ...
                {mostrarFormulario && (
                    <form onSubmit={handleSubmit}>
                        ...
                        <button type="submit" className="btn btn-primary">{editandoProducto ? 'Editar Producto' : 'Agregar Producto'}</button>
                    </form>
                )}
                        ...
                        <tbody>
                            {productos.map(producto => (
                                <tr key={producto.Id_producto}>
                                    ...
                                    <td>
                                        <button onClick={() => handleEditarProducto(producto.Id_producto)} className="btn btn-secondary">Editar</button>
                                    </td>
                                </tr>
                            ))}
                            <tr>
